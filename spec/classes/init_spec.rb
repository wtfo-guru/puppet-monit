    ...
    platforms.sort.each do |k, v|
      context k.to_s do
        ...
        if v[:osfamily] == 'Debian'
          it { is_expected.to contain_file('/etc/default/monit').with('notify' => 'Service[monit]') }

          it { is_expected.to contain_file('/etc/default/monit').with_content(%r{^#{v[:default_content]}$}) }
        else
  ...
  describe 'failures' do
    ...
    context 'when major release of Debian is unsupported' do
      ...
      it 'fails' do
        expect {
          is_expected.to contain_class('monit')
        }.to raise_error(Puppet::Error, %r{monit supports Debian 6 \(squeeze\), 7 \(wheezy\) and 8 \(jessie\) and Ubuntu 10\.04 \(lucid\), 12\.04 \(precise\), 14.04 \(trusty\) and 16.04 \(xenial\). \
