    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        when 'Debian'
          ...
          when 'wheezy', 'jessie', 'stretch', 'precise', 'trusty', 'xenial'
  ...
  describe 'failures' do
    ...
    context 'when major release of Debian is unsupported' do
      ...
      it 'fails' do
        expect {
          is_expected.to contain_class('monit')
        }.to raise_error(Puppet::Error, %r{monit supports Debian 6 \(squeeze\), 7 \(wheezy\), 8 \(jessie\) and 9 \(stretch\) and Ubuntu 10\.04 \(lucid\), 12\.04 \(precise\), 14.04 \(trusty\) \
and 16.04 \(xenial\). Detected lsbdistcodename is <etch>\.})
      end
    end
...
and 16.04 \(xenial\). Detected lsbdistcodename is <hardy>\.})
      end
    end
